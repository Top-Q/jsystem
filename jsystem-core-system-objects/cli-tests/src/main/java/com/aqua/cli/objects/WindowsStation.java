 ...
 *
 *	This class implements the commands for Windows based platform 
 *  and runing them using the CliApplication System Object 
 */

public class WindowsStation extends CliApplication implements Station {

	/**
	 * This method running dir command in CLI and check error messages tht can be thrown
	 * 
	 * @param folderName
	 */
	public void dir(String folderName) throws Exception {
		CliCommand cmd = new CliCommand();
		cmd.setCommands(new String[]{"dir " + folderName});
		cmd.addErrors("File Not Found");
		cmd.addErrors("not recognized");
	...
	public void ipConfig()throws Exception {
		CliCommand cmd = new CliCommand();
		cmd.setCommands(new String[]{"ipconfig"});
		cmd.addErrors("incomplete command line");
		cmd.addErrors("unrecognized");
		cmd.addErrors("not recognized");
	...
	public void ping(String hostToPing)throws Exception {
		CliCommand cmd = new CliCommand();
		cmd.setCommands(new String[]{"ping " + hostToPing});
		cmd.addErrors("could not find");
		cmd.addErrors("Request timed out");
		cmd.addErrors("not recognized");
	...
	public void runCommandsArray()throws Exception {
		CliCommand cmd = new CliCommand();
		cmd.setCommands(new String[]{"dir","ipconfig","ping"});
		cmd.addErrors("not recognized");
		cmd.addErrors("File Not Found");
	...
	public void runCommand(String[] commands, String[] errors)throws Exception {
		...
		handleCliCommand("Running commands with errors implemented...", cmd);
	}
	...
	public void runCommandsNoEnter() throws Exception {
		CliCommand cmd = new CliCommand();
		cmd.setCommands(new String[]{"dir"});
