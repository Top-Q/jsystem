...
public class ObjectArrayParameterProvider extends AbstractSerializingParameterProvider {
	...
	public Object getFromString(String stringRepresentation) throws Exception {
		...
		String propertiesString = stringRepresentation
				.substring(classEndIndex + 1);
		Properties properties = new Properties();
		try {
			properties.load(new StringReader(propertiesString));
		} catch (IOException e1) {
			log.log(Level.WARNING, "Fail to load properties: "
					+ propertiesString, e1);
			return null;
		}
		...
		Object[] array = (Object[]) Array
				.newInstance(c, splitProperties.size());
		for (int i = 0; i < array.length; i++) {
			array[i] = BeanUtils.propertiesToObject(c,
					propertiesToMap(splitProperties.get(i)));
		}
		return array;
	}
	...
	@Override
	public synchronized Object showUI(Component parent, Scenario currentScenario,
			RunnerTest rtest, Class<?> classType, Object object,Parameter parameter) throws Exception {
		if (!classType.isArray()){
			throw new RuntimeException("ObjectArrayParameter must be of array type! current type is : " + classType);
		}
		ArrayList<BeanElement> beanElements = BeanUtils.getBeans(classType
				.getComponentType(), true,true, BeanUtils.getBasicTypes());
		...
		if (object != null) {
			...
			try{
				...
				throw new RuntimeException("ObjectArrayParameter got wrong parameter! expected array type, got: " + object.getClass() + ", with value: " + object);
			}

			for (int i = 0; i < array.length; i++) {
				Properties oProperties = BeanUtils.objectToProperties(array[i],
						beanElements);
				multiMap
						.add(propertiesToMapBeanOrder(oProperties, beanElements));
			}
		}
		// the user can specified using <code>TestBeanClass</code> annotation new data model class.
		...
		MultiPropertiesDialog dialog = new MultiPropertiesDialog(
				multiMap, 
				"Bean properties",
				beanElements, 
				propertiesToMapBeanOrder(
						BeanUtils.objectToProperties(classType.getComponentType()
								.newInstance(), beanElements), beanElements), beanCellEditorModel
		...
		if (dialog.showAndWaitForApprove()) {
			Object[] array = (Object[]) Array.newInstance(classType.getComponentType(),
					multiMap.size());
			for (int i = 0; i < array.length; i++) {
				array[i] = BeanUtils.propertiesToObject(classType.getComponentType(),
						multiMap.get(i));
			}
			return array;
		}
	...
	private static LinkedHashMap<String, String> propertiesToMapBeanOrder(
			Properties properties, ArrayList<BeanElement> elements) {
		LinkedHashMap<String, String> map = new LinkedHashMap<String, String>();
