...
public class AntForLoop extends AntFlowControl {
	...
	private static Pattern LOOP_NUMBER_PATTERN = 
		Pattern.compile("(\\d*)\\.\\.(\\d*)((\\:(\\d*))|\\z)",Pattern.CASE_INSENSITIVE);

	// XML strings attributes, as we will use in the created XML
	// Each of the attributes will have its relevant "creator" according to the definition
	// of the task
	...
	public static AntForLoop fromElement(JTestContainer parent, Element element) {
		AntForLoop newContainer = new AntForLoop(parent, null);
		
		String listValue = newContainer.list.getValue().toString();
		listValue = element.getAttribute("list").toString();
		newContainer.setLoopValuesList(listValue);

		String paramValue = newContainer.param.getValue().toString();
		paramValue = element.getAttribute("param").toString();
		newContainer.param.setValue(paramValue);
	...
	public Element addExecutorXml(Element targetScenario, Document doc) {
		...
		echo2.setAttribute("message", "Parameter: " + param.getValue().toString() + "=@{" + param.getValue().toString() + "}");
		...
		XmlUtils.appendComment(setAntProperties, "Task for updating the ant parameters file - used for reference parameters");
	...
	/**
	 * Two examples which explain what the method does:
	 * if the user enters 1..10:2 the text will be replaced with 1;3;5;7;9;
	 * if the user enters 10..1:3 the text will be replaced with 10;7;4;1;
	 */
	...
	/**
	 * get the number of loops for this loop, according to the loop values String<br>
	...
	/**
	 * get the value the loop runs on for a given loop number (first loop is 1, 0 is out of bounds)
	 * 
	 * @param loopNum	the loop number to get loop for
	 * @return	the loop variable value or "DYNAMIC VALUE" if values is a reference (Dynamic)
	 */
