...
public class XmlReportHandlerTests {
	...
	public void before() throws Exception {
		URL url = this.getClass().getResource("/reports.0.xml.orig");
		Assert.assertNotNull(url);
		File orig = new File(url.getFile());
		reportFile = new File(orig.getParentFile(), "reports.0.xml");
		FileUtils.copyFile(orig, reportFile);
		handler = new XmlReportHandler(reportFile.getParentFile());
	}
